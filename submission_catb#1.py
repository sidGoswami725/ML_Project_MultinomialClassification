...
from catboost import CatBoostClassifier # Switched to CatBoost
...
TEST_FILE = "test.csv"
SUBMISSION_FILE = "submission_catboost_final.csv"
SEED = 42
...
clf_cat = CatBoostClassifier(
    iterations=500, # Using 500 for quick comparison
    learning_rate=0.05,
    depth=6,
    loss_function='MultiClass',
    auto_class_weights='Balanced', # Built-in balancing
    random_seed=SEED,
...
# Validation F1-Score
val_pred = clf_cat.predict(X_valid_split).flatten() # CatBoost output needs flattening
f1 = f1_score(y_valid_split, val_pred, average='weighted')
print(f"CatBoost Weighted F1-Score (Validation) = {f1:.4f}")
