...
TEST_FILE = "test.csv"
SUBMISSION_FILE = "submission_xgb_final.csv"
SEED = 42
...
def preprocess_data(train_path, test_path):
    ...
    # 1. Total People (Proxy for group size spending power)
...
# --- 2. MODEL DEFINITION & TRAINING (XGBoost) ---
...
# Initialize XGBoost
# We use 'multi:softmax' for hard-label prediction, as required by the submission format
...
# Validation F1-Score
val_pred = clf_xgb.predict(X_valid_split)
f1 = f1_score(y_valid_split, val_pred, average='weighted')
print(f"XGBoost Weighted F1-Score (Validation) = {f1:.4f}")
...
test_pred = clf_xgb.predict(test_df).astype(int)
